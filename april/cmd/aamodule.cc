...
/* ------------------------------------------------------------------------- */
AaModule::AaModule	( CommandMap * cmd_map )
...
/* ------------------------------------------------------------------------- */
QString		AaModule::getCLUsage				( const QString & s_name )
{
	QString s_usage;
...
/* ------------------------------------------------------------------------- */
bool		AaModule::funcArg0					(
		const QString & s_cmd, const AaTkString & atks, 
		QString & s_err, AaModule::arg0Func kb )
{
...
/* ------------------------------------------------------------------------- */
bool		AaModule::funcArg1					(
		const QString & s_cmd, const AaTkString & atks, 
		QString & s_err, AaModule::arg1Func kb )
{
	Q_ASSERT( atks.tk_.count() >= 1 );
	Q_ASSERT( kb != NULL );
	int arg_cnt = atks.tk_.count() - 1;
